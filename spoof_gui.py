...
class spoofGUI(Tkinter.Tk):
	...
	def initialize(self):
		...
		#Header
		label = Tkinter.Label(self, text = "Spoof GUI!", font=("Helvetica", "20"), padx="20", pady="10", justify="center")
		...
		def refreshInfoFrame():
			for i in range(0, len(self.infolist), 3):
				if self.target in self.infolist[i]:
					...
					# testL.config(text=self.infolist[i] + " - Device: " + self.infolist[i+1])
					# testL.grid(row=(0), column=0, padx=20, sticky='W')
					testLMac.config(text="Orignal MacAddress:\nCurrent MacAddress:")
					# testLMac.grid(row=(0), column=1, pady=5)
					testLMacNo.config(text=self.infolist[i+2] + '\n' + mAddress)
					# testLMacNo.grid(row=(0), column=2, pady=5)
					# testLMacNew = Tkinter.Label(infoFrame, text="\nCurrent MacAddress: " + mAddress)
					# testLMacNew.grid(row=((i/3)+1), column=1, pady=10)
		refreshInfoFrame()
		...
		def timedRandomize():
			...
			# 	tkMessageBox.showinfo("Randomize", "Please select a target above")
			# else:
			# 	spoof_mac_random(self.targetDevice)
			# 	refreshInfoFrame()
		...
		# labelframe = Tkinter.LabelFrame(tFrame, text="Randomize Timer")
		# labelframe.gid(row=0, column=0)
		...
		# self.macL1 = Tkinter.Label(macFrame, text="MacAddress")
		# self.macL1.grid(row=0, column=0)

		# self.macT1 = Tkinter.StringVar()
		# self.macE1 = Tkinter.Entry(macFrame, textvariable=self.macT1, width="15")
		# self.macE1.grid(row=0, column=1)
		# self.macT1.set("xx:xx:xx:xx:xx:xx")

		# self.macB1 = Tkinter.Button(macFrame, text="Set MacAddress", command=setMac)
		# self.macB1.grid(row=0, column=2)
